...
public class NumberUtils {
	...
	// -----------------------------------------------------------------------
	/**
	 * 判断字符串是否是数字0 1.空字符串表示0 2. 如.5表示非0
	 * 
	 * @param value
	 * @return
	 */
	public static boolean isZero(String value) {
		...
		if (dotIndex < 0) {
			// 不存在小数点
			return (Character.isDigit(value.charAt(0)) && Integer
					.parseInt(value) == 0);
		} else if (dotIndex == 0) {
			// 以小数点开头，如".5"
			if (value.length() == 1) {
				...
				// 判断小数部分是否是0
				return (Integer.parseInt(value.substring(1)) == 0);
			}

		} else {
			// 存在小数位数
			// 判断整数部分
			if (!isDigits(value.substring(0, dotIndex))
					|| !isDigits(value.substring(dotIndex + 1))) {
				// 整数部分或小数部分非数字
				return false;
			} else {
				// 判断整数部分
				if (Integer.parseInt(value.substring(0, dotIndex)) != 0) {
					// 整数部分大于0
					return false;
				} else {
					// 整数部分是0，判断小数部分
					if (Integer.parseInt(value.substring(dotIndex + 1)) != 0) {
						// 小数部分非0
						return false;
					} else {
						return true;
	...
	/**
	 * 判断一个字符串数字是否是负数，备注，假设此字符串确实是数字 判断标准为 1. 第一个字符为"-",则表示负数
	 * 
	 * @param value
	 * @return
	 */
